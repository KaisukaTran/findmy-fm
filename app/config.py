...
class Settings(BaseSettings):
    ...
    intraday_max_bars: int = Field(default=3000, description="Hard cap on candles fetched per symbol when backtest_timeframe is INTRADAY (below 1d). backtest_lookback_days is a calendar window: 365 days is 365 daily bars but 105,120 five-minute bars, and Binance silently returns only its first 1000 — so without a cap the backtest would run on 3.5 days of data while every label said a year. The cap keeps the window honest AND bounds the paging cost (3000 5m bars ≈ 10 days ≈ 3 requests per symbol). Ignored on daily/weekly timeframes. 0 = no cap (expect very slow scans and heavy exchange weight).")
    ...
    autotune_enabled: bool = Field(default=True, description="Let the app correct entry/exit settings that CONTRADICT each other, instead of trading nothing in silence. Today it enforces two arithmetic facts: min_expectancy_pct must sit under the ceiling a given scan_tp_pct can ever produce (tp minus round-trip cost — a gate above it skips 100%% of the universe forever, which cost 12h of paper and a live soak), and scan_tp_pct must clear the round-trip fee floor. It only ever RELAXES an impossible gate, never tightens one, and every change is logged in Nhật ký with its reason. False = leave a broken configuration exactly as set.")
    autotune_levels_enabled: bool = Field(default=False, description="Stage 2: let each coin get its OWN take-profit and DCA step, derived from its realised daily ATR%%, instead of one global pair of numbers for the whole universe (measured spread on our own watchlist: 3.9%%/day BTC to 8.6%%/day XRP — against which a fixed 3.24%% TP is ~0.45 ATR, handing most of a normal day back, and a fixed 2%% DCA step is ~0.28 ATR, close enough that ordinary noise walks the ladder down). Multiples are autotune_tp_atr_mult / autotune_dca_atr_mult, always clamped and never under the fee floor, and applied to NEW sessions only. OFF by default: it changes what the strategy does, so it is an explicit choice.")
    autotune_learn_enabled: bool = Field(default=False, description="Stage 3: let the app LEARN the take-profit ambition from closed sessions. Stage 2 derives the target from volatility, which is a prediction; this checks it against what happened — a run of sessions timing out means the target sits too far out, while targets hit almost immediately mean we sold back into a move that had barely started. Moves autotune_tp_atr_mult by one small step per run, only after enough closed sessions to be a pattern rather than noise, always inside hard clamps, and writes the evidence to Nhật ký. It never learns the RISK gates. Needs autotune_levels_enabled. OFF by default.")
    autotune_learn_interval_hours: float = Field(default=6.0, description="How often stage 3 reviews closed sessions and nudges autotune_tp_atr_mult. Slow on purpose: re-tuning on every result chases noise, and a target that swings around is worse than one slightly wrong.")
    autotune_tp_atr_mult: float = Field(default=0.8, description="Stage 2: take-profit as a multiple of the coin's own daily ATR%%. One fixed TP cannot suit a universe whose ATR runs 3.9%%/day (BTC) to 8.6%%/day (XRP) — measured. At 0.8 a coin moving 7%%/day targets ~5.6%%, about a day's work; the old fixed 3.24%% was ~0.45 ATR, i.e. handing most of a normal day back. Never lands below the round-trip fee floor, never above 15%%. Applies to NEW sessions only.")
    autotune_dca_atr_mult: float = Field(default=0.5, description="Stage 2: DCA step as a multiple of the coin's own daily ATR%%. A step far under the daily range gets walked down by ordinary noise (the old fixed 2%% was ~0.28 ATR); too wide and the ladder never fills. Clamped to 0.5–10%%. Applies to NEW sessions only.")
    ...
    grok_scanner_shadow: bool = Field(default=False, description="ĐO KHÔNG CAN THIỆP: vẫn hỏi Grok và GHI phán quyết vào candidates.grok_verdict, nhưng KHÔNG cho nó chặn hay mở coin nào — scanner quyết định y như khi Grok tắt. Đây là cách duy nhất trung thực để biết Grok có giỏi hơn công thức không: không thể back-test một LLM (hỏi nó về quá khứ thì nó có thể đã biết kết quả), nên phải đo tiến về phía trước với một nhánh đối chứng. Chấm điểm bằng scripts/grok_shadow_eval.py. Vẫn TỐN tiền API như chạy thật.")
    ...
    kss_stop_ratchet_step_pct: float = Field(default=0.5, ge=0.05, le=10, description="Chỉ thay lệnh stop trên sàn khi stop mới cao hơn stop đang đặt ít nhất % này. Huỷ lệnh KHÔNG hoàn lại quota lệnh chưa khớp của Binance, nên thay quá dày là đường ngắn nhất tới lỗi -1015.")
    kss_stop_limit_slip_pct: float = Field(default=0.3, ge=0.0, le=5, description="STOP_LOSS_LIMIT: giá limit đặt thấp hơn giá kích hoạt % này, để lệnh vẫn khớp khi giá xuyên nhanh.")
    kss_stop_max_replaces: int = Field(default=40, ge=0, le=500, description="Trần số lần thay lệnh stop cho MỘT phiên. Chạm trần thì giữ nguyên stop cũ (vẫn bảo vệ), không thay nữa.")
