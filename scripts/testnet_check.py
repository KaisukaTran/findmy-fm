...
def check_private(ex) -> None:
    """Confirm the keys are accepted and show what the testnet faucet funded."""
    ...
    if free:
        shown = sorted(free.items(), key=lambda kv: -kv[1])[:8]
        _say(OK, "funded: " + ", ".join(f"{k}={v:g}" for k, v in shown))
    else:
        _say(WARN, "no free balance — testnet resets wipe funds; regenerate keys to get a "
                   "freshly funded account")


def check_place(ex, symbol: str, last: float, filters: dict,
...
def main() -> int:
    ...
    try:
        ...
        check_private(ex)
        if args.place:
            check_place(ex, args.symbol, last, filters, args.notional, args.distance_pct)
        else:
